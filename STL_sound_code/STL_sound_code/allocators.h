...
#define SYNC_DEFAULT stdext::allocators::sync_shared
		...
		public:
			...
			{	// compare two caches for equality
				return (_Mycache_ptr != nullptr
					&& _Other._Mycache_ptr != nullptr
		...
		public:
			cache_chunklist()
				: _List(nullptr), _Cached_block(nullptr)
			{	// construct with empty list
			}

			~cache_chunklist() noexcept
		...
		private:
			...
			public:
				...
				{	// construct list
					_Fl = &_Data[0];
					_Free_count = _Nelts;
					for (int _Ix = 0; _Ix < _Nelts - 1; ++_Ix)
