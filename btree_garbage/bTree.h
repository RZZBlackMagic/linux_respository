...
//	cout<<"left_node:"<<left_node->num<<endl;	
	...
	right_node->childs[right_node->num] = node->childs[node->degree];
//	cout<<"right_node:"<<right_node->num<<endl;
	//处理父节点
...
//		cout<<insert_place_node->leaf<<endl;
		//此时插入的节点一定在叶节点
		b_tree_node<T>* new_node = new b_tree_node<T>(4,insert_place_node);
		cout<<"****************************"<<endl;
...
//			cout<<"x:"<<x<<endl;
			...
			for(int i=0;i<new_node->parent->num+1;i++){
				if(insert_place_node->parent->childs[i]==insert_place_node){
					cout<<"77"<<endl;
