...
//                        std::cout<<"obj对象个数："<<cur->_obj_size<<std::endl;
...
//	std::cout<<"deallocate span->pageid:"<<((PageID)start>>12)<<std::endl;
//	std::cout<<"deallocate span->objlist:"<<span->_objlist<<std::endl;
...
//	std::cout<<"对应的freelist的index："<<span->_obj_size<<std::endl;
//	std::cout<<"对应的freelist的size："<<_freelist[_index].size()<<std::endl;
	_freelist[_index].push(start);//回收自由链表的一块，[start,NEXT_OBJ(start)];
	//std::cout<<"回收后thread的自由链表"<<std::endl;
	//printThreadCache();
	//2 如果链表的长度大于max_size，则回收到central
	//为什么要大于max_size再回收，因为一次申请一个size大小的内存。但是分配max_size个，所以当大于max_size时，至少申请了两次，里面最少有2×maxsize个
	//但是只需要一个，内存浪费严重，所以回收
	std::cout<<"size:"<<_freelist[_index].size()<<std::endl;
	std::cout<<"max size:"<<_freelist[_index].max_size()<<std::endl;
